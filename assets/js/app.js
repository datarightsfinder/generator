...
$(function() {
  ...
  $('body').on('click', 'legend', function(e) {
    ...
    if (
      ...
      $('input[name=organisationInformation_number]').val() === '' ||
      $('select[name=organisationInformation_registrationCountry]').val() === ''
    ) {
      return;
    }
    ...
    if (
      selectedCountry !== 'us' &&
      $(this).parent().attr('data-alpaca-field-name') === 'privacyShield') {
      return;
    }
    ...
    if (container.attr('data-open') === 'true') {
      ...
      $(this).parent().find('.form-group, .alpaca-array-toolbar, ' +
        '.alpaca-helper, .alpaca-array-actionbar, .pull-left, .pull-right, ' +
        'legend, .form-custom-button').hide();
      $(this).parent().find('legend').eq(0).show();
    } else if (
      container.attr('data-open') === 'false' || !container.attr('data-open')) {
      // Open
  ...
  function setupForm(form) {
    ...
    setTimeout(function() {
      ...
      // Hide all fields
      $('.form-group, .alpaca-array-toolbar, .alpaca-helper, ' +
        '.alpaca-array-actionbar, .pull-right, .form-custom-button, ' +
        '#mainForm div div div div div').hide();
  ...
  $('body').on('keyup', '#mainForm', function(e) {
    if (e.which === 9 && $(document.activeElement).parent()
      .hasClass('alpaca-container-label')) {
  ...
  function refreshContributeForm() {
    ...
    $('.number').each(function(i, e) {
      ...
      var textareas = $(this).parent().parent().parent().parent()
        .find('textarea');
    ...
    if (selectedCountry !== 'us') {
      $('[data-alpaca-container-item-name="privacyShield"] legend a')
        .attr('style', '');
    }
